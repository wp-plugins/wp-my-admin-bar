...
<?php if ( !isset( $_GET['tab'] ) || ( $_GET['tab'] == "menu_settings" ) ) {?>
				...
				<div  class="postbox">
					<h3><span><?php _e('Option Data', 'wp-my-admin-bar');?></span></h3>
						<div class="inside">
						<p><?php _e('Below is the Admin Bar Option Values that this Website has within the database.', 'wp-my-admin-bar');?></p>
							<ul>
								...
								<?php if ( !$not_multisite ) {?><li><strong><?php _e('Transient Cache', 'wp-my-admin-bar');?></strong>: <?php if ( get_site_transient( 'multisite_site_list' ) ) { _e('In Use', 'wp-my-admin-bar'); }else{ _e('Empty', 'wp-my-admin-bar'); } ?></li><?php }?>
							</ul>
						</div> <!-- end inside -->
				</div> <!-- end postbox -->

<?php }else{ /* Standalone Wordpress */
		if ( !is_plugin_active_for_network( MYAB_PLUGIN_BASENAME ) ) {
?>
				<div  class="postbox">
					<h3><span><?php _e('Custom Settings', 'wp-my-admin-bar');?></span></h3>
						<div class="inside">
							<p>These settings adjust this Website only.</p>
							<form method="post" action="">
							<?php wp_nonce_field('my_custom_action','my_custom_nonce');?>
								<p><strong><?php _e('Display Wordpress Logo on Admin Bar', 'wp-my-admin-bar');?></strong>: <input class="valinp" type="radio"  name="wplogo" value="show" <?php echo $wplogo_on;?> /> [<?php _e('show', 'wp-my-admin-bar');?>]&nbsp;&nbsp;&nbsp;<input class="valinp" type="radio" name="wplogo" value="hide" <?php echo $wplogo_off;?> /> [<?php _e('hide', 'wp-my-admin-bar');?>]</p>
								<p><strong><?php _e('Display Howdy, Handle on Admin Bar', 'wp-my-admin-bar');?></strong>: <input class="valinp" type="radio"  name="howdy" value="show" <?php echo $howdy_on;?> /> [<?php _e('show', 'wp-my-admin-bar');?>]&nbsp;&nbsp;&nbsp;<input class="valinp" type="radio" name="howdy" value="hide" <?php echo $howdy_off;?> /> [<?php _e('hide', 'wp-my-admin-bar');?>]</p>
								<?php if ( $not_multisite ) {?><p>Multisite Features:</p><?}?>
								<p><strong><?php _e('Display WP Icon in My Sites/Cache Menu', 'wp-my-admin-bar');?></strong>: <input class="valinp" type="radio"  name="wpicon" value="show" <?php echo $wpicon_on; echo $not_multisite;?> /> [<?php _e('show', 'wp-my-admin-bar');?>]&nbsp;&nbsp;&nbsp;<input class="valinp" type="radio" name="wpicon" value="hide" <?php echo $wpicon_off; echo $not_multisite;?> /> [<?php _e('hide', 'wp-my-admin-bar');?>]</p>
								<p><strong><?php _e('Display Site IDs in My Sites Menu', 'wp-my-admin-bar');?></strong>: <input class="valinp" type="radio"  name="siteids" value="show" <?php echo $siteids_on; echo $not_multisite;?> /> [<?php _e('show', 'wp-my-admin-bar');?>]&nbsp;&nbsp;&nbsp;<input class="valinp" type="radio" name="siteids" value="hide" <?php echo $siteids_off; echo $not_multisite;?> /> [<?php _e('hide', 'wp-my-admin-bar');?>]</p>
